...
boolean focus_Timer_Flag = false;
...
unsigned long prevMillisFocus = 0;
unsigned long prevMillisFocusCount = 0;
int focusSec = 0;
int focusMin = 0;
#define PIN D5
#define PIN_VIB D6
...
byte LEDpins[NBR_EVENTS] = {D2, 100, D4, D8};   // connect LEDs to these pins or change pin number here
  ...
  for (int i = 0; i < NBR_EVENTS; i++)
  {
    ...
    case none:
      if (switchPressed[i])
      {
        ...
        while (!calenderUpToDate)
          getCalendar();
        if (!eventHere(i))
          createEvent(possibleEvents[i]);
        Serial.print(i);
        Serial.println(" 0 -->1");
        // getCalendar();
        taskStatus[i] = due;
...
void neoRestFade()
{
  ...
  for (i = 0; i <= matrix.numPixels(); i++)
  {
    matrix.setPixelColor(i, 204, 0, 204);
  }
...
{
  //#########################  FOCUS MODE  #########################33
  if (digitalRead(FOCUS_PIN) == true) // REad for touch button
  {
    prevMillisFocus = millis();
  }
  else
  {

    if (millis() - prevMillisFocus > 200 && digitalRead(FOCUS_PIN) == LOW)
    {
      Serial.println("Start Focus Timer");
      focus_Timer_Flag = !focus_Timer_Flag;
      delay(2000);
      prevMillisFocus = millis();
    }
  }

  if (focus_Timer_Flag == true && millis() - prevMillisFocusCount > 1000)
  {
    focusSec++;
    Serial.println("Focus Timer" + (String)focusSec);
    Serial.println("Focus Mins: " + (String)(focusSec / 60));
    Serial.println("Focus Sec: " + (String)(focusSec % 60));
    String timer = "Timer " + (String)(focusSec / 3600) + ":" + (String)(focusSec / 60) + ":" + (String)(focusSec % 60)+"      "+"FM: "+(String)focus_time+"  RM: "+(String)rest_time;
    lcdf.print(0, 0, timer);
    prevMillisFocusCount = millis();
    if(focusSec<focus_time*60)
    {
    neoFocusFade();
    }
else if(focusSec>=focus_time*60 && focusSec<(rest_time+focus_time)*60)
{
  neoRestFade();
}
if(focusSec>focus_time*60 && focusSec>(rest_time+focus_time)*60)
{
  //focus_Timer_Flag=false;
  focusSec=0;
...
{
  Blynk.run();
  if (millis() > entryCalender + UPDATETIME && focus_Flag == false)
  {
    getCalendar();
    entryCalender = millis();
    getNTP();
  ...
  if (startTimer == true && focus_Flag == false)
  {
    neoFriendFade();
    startTimer = false;
  }

  if (focus_Flag == true) // BLYNK FOCUS BUTTON CLICKED
  {
    focusMode();
  } // Focus flag terminator
}
...
BLYNK_WRITE(V5)
{
  ...
  if (inp == false)
  {
    Serial.println("Turn OFF focus Mode");
    Blynk.setProperty(V5, "offLabel", "OFF");
    focus_Flag = false;
  }
  else
  {
    Serial.println("Turn ON focus Mode");
...
BLYNK_WRITE(V4)
{
  focus_time = param.asInt();
  Serial.println("focus_Time" + (String)param.asInt());
  String timer = "Timer " + (String)(focusSec / 3600) + ":" + (String)(focusSec / 60) + ":" + (String)(focusSec % 60)+"      "+"FM: "+(String)focus_time+"  RM: "+(String)rest_time;
    lcdf.print(0, 0, timer);
}
...
BLYNK_WRITE(V7)
{
  rest_time = param.asInt();
  Serial.println("REst_Time" + (String)param.asInt());
  String timer = "Timer " + (String)(focusSec / 3600) + ":" + (String)(focusSec / 60) + ":" + (String)(focusSec % 60)+"      "+"FM: "+(String)focus_time+"  RM: "+(String)rest_time;
    lcdf.print(0, 0, timer);
}
